...
def gen_mark(args):
    ...
    def mark_im(im):
        ...
        while y < c:
            # 制造x坐标错位
            x = -int((mark.size[0] + args.space)*1.5*idx)
            idx = (idx + 1) % 2
...
def main():
    ...
    parse.add_argument("-c", "--color", default="#ffffff", type=str, help="text color like '#000000', default is #ffffff")
    parse.add_argument("-s", "--space", default=300, type=int, help="space between watermarks, default is 75")
    parse.add_argument("-a", "--angle", default=30, type=int, help="rotate angle of watermarks, default is 30")
    parse.add_argument("--size", default=70, type=int, help="font size of text, default is 50")
    parse.add_argument("--opacity", default=0.16, type=float, help="opacity of watermarks, default is 0.15")

    args = parse.parse_args()

    if isinstance(args.mark, str) and sys.version_info[0] < 3:
        print chardet.detect(args.mark), args.mark
